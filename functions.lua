...
-- connection-glue fling: stick to the target with an AlignPosition constraint
-- (resolved by physics every step -> tracks their LIVE position continuously, no
-- TP chase) and spin violently with an AngularVelocity constraint. The sustained
-- glued contact + spin flings them. You actually move onto them, then snap back.
local function flingPlayer(plr)
    ...
    task.spawn(function()
        local hrp=lhrp
        local hum=char:FindFirstChildOfClass("Humanoid")
        -- PlatformStand so the humanoid stops fighting the spin
        if hum then pcall(function() hum.PlatformStand=true end) end
        -- the "connection": Attachment + constraints on our own HRP (we own it)
        ...
        av.Attachment0=att; av.RelativeTo=Enum.ActuatorRelativeTo.World
        av.MaxTorque=math.huge; av.AngularVelocity=Vector3.new(1,1,1)*120
        av.Parent=hrp
        local hb
        hb=RunService.Heartbeat:Connect(function()
            local tc=target.Character; local th=tc and tc:FindFirstChild("HumanoidRootPart")
            if not th or not ap.Parent then return end
            ap.Position=th.Position   -- glue to their live position every step
        end)
        local deadline=tick()+2.5
        while tick()<deadline do
            if not target.Character or not lplr.Character or not char.Parent then break end
            task.wait()
        end
