...
-- ============================================================
--  HOOD CUSTOMS: SILENT AIM (raycast redirect, spread-preserving)
-- ============================================================
-- Hooks the gun's hit raycast (spotted by its character blacklist) and
-- ROTATES each pellet's direction so the spread cone re-centers on the
-- target while every pellet keeps its offset - the game still fires the
-- remote itself (real origin / spread / GunCheck), only the cone's aim
-- changes. Preserving the spread shape is the attempt at HC's "spoofing
-- spread pattern" check (decay collapsed the cone -> got flagged).
-- The engine returns the REAL hit, so a wall in the way is still a wall
-- (this is silent aim on visible targets, not a wallbang).
hook.games.hoodCustoms.silentRay = (function()
    local Players = game:GetService("Players")
    ...
    local function targetPart()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            local part = ch and (ch:FindFirstChild(hitPartName) or ch:FindFirstChild("HumanoidRootPart"))
            local hum  = ch and ch:FindFirstChildOfClass("Humanoid")
            if part and hum and hum.Health > 0 then
                local sp, on = cam:WorldToViewportPoint(part.Position)
                if on then
                    local d = (Vector2.new(sp.X, sp.Y) - mp).Magnitude
                    if d <= fovPx and (not bestD or d < bestD) then best, bestD = part, d end
                end
            end
        end
        return best
    ...
    local function install()
        if installed or getgenv()._HC_SILENTRAY then return end
        installed = true
        getgenv()._HC_SILENTRAY = true
        local old
        old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            -- only while firing, only the gun's long, character-blacklisted ray
            if active and not checkcaller() and getnamecallmethod() == "Raycast"
                and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                local origin, dir, params = ...
                if typeof(origin) == "Vector3" and typeof(dir) == "Vector3"
                    and typeof(params) == "RaycastParams" and dir.Magnitude > 50 then
                    local ch, mine = lp.Character, false
                    if ch then
                        for _, inst in ipairs(params.FilterDescendantsInstances) do
                            if inst == ch then mine = true break end
                        end
                    end
                    if mine then
                        local tp = targetPart()
                        if tp then
                            local cam  = workspace.CurrentCamera
                            local aim  = cam and cam.CFrame.LookVector or dir.Unit
                            local want = tp.Position - origin
                            if want.Magnitude > 1e-3 then
                                want = want.Unit
                                local axis = aim:Cross(want)
                                local dot  = math.clamp(aim:Dot(want), -1, 1)
                                if axis.Magnitude > 1e-4 then
                                    local rot = CFrame.fromAxisAngle(axis.Unit, math.acos(dot))
                                    return old(self, origin, rot * dir, params)
                                end
                            end
                        end
                    end
    ...
    return {
        start      = function() install(); active = true end,
        stop       = function() active = false end,
        setHitPart = function(n) hitPartName = tostring(n or "Head") end,
        setFov     = function(n) fovPx = math.clamp(tonumber(n) or 250, 10, 2000) end,
    }
end)()
...
Combat:NewSection("Silent aim (raycast)")
-- redirects the gun's own raycast so the game fires a legit shot at the
-- target. spread-preserving (cone re-centered, not collapsed). visible
-- targets only - it can't shoot through walls. tests HC's pattern check.
regToggle(Combat, "HC_SilentRay", "Silent aim (raycast redirect)", false, function(v)
    if v then hc.silentRay.start() else hc.silentRay.stop() end
end)
regDropdown(Combat, "HC_SilentRayPart", "Hit part", "Head", { "Head", "UpperTorso", "HumanoidRootPart" }, false, function(v) hc.silentRay.setHitPart(v) end)
regSlider(Combat, "HC_SilentRayFov", "FOV (px)", "", { min = 10, max = 1000, default = 250 }, function(v) hc.silentRay.setFov(v) end)
