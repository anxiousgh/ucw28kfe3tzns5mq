...
-- (void/sky/jitter - the real position leaks into replication, we filter it out
-- and reposition their model on YOUR client). It can't recover a CLEAN desync
-- (real pos never sent) and it can't STOP their desync server-side - that's
-- between their client and the server; this only fixes how they look to you.
Desync:NewSection("Desync resolver")
do
    local RunSvc3  = game:GetService("RunService")
    local PlayersS = game:GetService("Players")
    local LP       = PlayersS.LocalPlayer
    local on   = false
    local hist = setmetatable({}, { __mode = "k" })   -- [player] = {items={}, lastSet=}
    local function chars()
        local t = {}
        for _, p in ipairs(PlayersS:GetPlayers()) do if p.Character then t[#t + 1] = p.Character end end
        return t
    ...
    local function keyOf(p) return math.floor(p.X / 4) .. "," .. math.floor(p.Y / 4) .. "," .. math.floor(p.Z / 4) end
    local function resolve(plr, hrp, ignore)
        local raw = hrp.Position
        local b = hist[plr]; if not b then b = { items = {} }; hist[plr] = b end
        local items = b.items
        -- record only genuine replication updates, not our own last correction
        if not b.lastSet or (raw - b.lastSet).Magnitude > 0.1 then
            items[#items + 1] = { p = raw, g = grounded(raw, ignore) }
            while #items > 40 do table.remove(items, 1) end
        end
        if #items < 6 then return nil end
        local freq, domKey, domN = {}, nil, 0
        for _, e in ipairs(items) do
            local k = keyOf(e.p); freq[k] = (freq[k] or 0) + 1
            if freq[k] > domN then domN, domKey = freq[k], k end
        end
        local excludeDom = domN >= #items * 0.4   -- one spot is 40%+ of frames -> spoof
        for i = #items, 1, -1 do
            if items[i].g and not (excludeDom and keyOf(items[i].p) == domKey) then return items[i].p end
        end
        for i = #items, 1, -1 do if items[i].g then return items[i].p end end
        return nil
    end
    regToggle(Desync, "DesyncResolver", "Desync resolver (snap players to real pos)", false, function(v)
        on = v
        if not v then for k in pairs(hist) do hist[k] = nil end end
    end)
    RunSvc3.RenderStepped:Connect(function()
        if not on or library.Unloaded then return end
        local ignore = chars()
        for _, p in ipairs(PlayersS:GetPlayers()) do
            local ch = p ~= LP and p.Character
            local hrp = ch and ch:FindFirstChild("HumanoidRootPart")
            if hrp then
                local resolved = resolve(p, hrp, ignore)
                -- only correct when clearly desynced (resolved far from the read)
                if resolved and (resolved - hrp.Position).Magnitude > 5 then
                    local b = hist[p]
                    pcall(function() hrp.CFrame = CFrame.new(resolved) * (hrp.CFrame - hrp.CFrame.Position) end)
                    if b then b.lastSet = resolved end
                end
            end
        end
    end)
