...
do
    ...
    local s = {
        target = nil,
        attached = false,
        tracer = { enabled = true, mode = "Mouse", thick = 0.05, color = Color3.fromRGB(255, 0, 0), rainbow = true },
        net = true,
    }
    local c, r
    local function setup(char)
        c = char
        r = char:WaitForChild("Humanoid").RootPart
    end
    if LP.Character then setup(LP.Character) end
    LP.CharacterAdded:Connect(setup)

    local b, a0, a1, hue = nil, nil, nil, 0
    local function beam()
        if b then return end
        a0 = Instance.new("Attachment", workspace.Terrain)
        a1 = Instance.new("Attachment", workspace.Terrain)
        b = Instance.new("Beam", workspace)
        b.Attachment0 = a0
        b.Attachment1 = a1
        b.FaceCamera = true
        b.LightEmission = 1
        b.LightInfluence = 0
        b.Segments = 128
        b.Texture = "rbxassetid://1217127707"
        b.TextureLength = 15
        b.TextureSpeed = 16
        b.TextureMode = Enum.TextureMode.Wrap
        b.Transparency = NumberSequence.new(0)
        b.Width0 = s.tracer.thick
        b.Width1 = s.tracer.thick * 0.5
    end
    local function closest()
        ...
        for _, p in ipairs(PlayersS:GetPlayers()) do
            if p ~= LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if mag < dist and vis then dist, best = mag, p end
            end
        end
        return best
    end
    local function own(part)
        if part and part:IsA("BasePart") and s.net then pcall(function() part:SetNetworkOwner(LP) end) end
    end

    Desync:NewKeybind("Target (closest to mouse)", Enum.KeyCode.Q, function()
    ...
    RunSvc.Heartbeat:Connect(function()
        if not s.attached or not c or not r then return end
        local tr = s.target and s.target.Character and s.target.Character:FindFirstChild("HumanoidRootPart")
        if not tr then s.target = nil; return end
        own(r)
        own(tr)
        pcall(function()
            if sethiddenproperty then sethiddenproperty(r, "PhysicsRepRootPart", tr) end
            r.CFrame = tr.CFrame
    ...
    RunSvc.RenderStepped:Connect(function(dt)
        if library.Unloaded or not s.tracer.enabled or not s.target then if b then b.Enabled = false end return end
        beam()
        local tr = s.target and s.target.Character and s.target.Character:FindFirstChild("HumanoidRootPart")
        if tr then
            if s.tracer.mode == "Mouse" then
                local m = UIS:GetMouseLocation()
                local ray = camera:ViewportPointToRay(m.X, m.Y, 0)
                a0.Parent = workspace.Terrain
                a0.WorldCFrame = CFrame.new(ray.Origin + ray.Direction * 10)
            elseif s.tracer.mode == "Bottom" then
                local vp = camera.ViewportSize
                local ray = camera:ViewportPointToRay(vp.X * 0.5, vp.Y - 5, 0)
                a0.Parent = workspace.Terrain
                a0.WorldCFrame = CFrame.new(ray.Origin + ray.Direction * 10)
            elseif s.tracer.mode == "HRP" and c and r then
                a0.Parent = r
                a0.Position = Vector3.new()
            end
            a1.Parent = tr
            b.Enabled = true
            if s.tracer.rainbow then hue = (hue + dt * 0.1) % 1; b.Color = ColorSequence.new(Color3.fromHSV(hue, 1, 1))
            else b.Color = ColorSequence.new(s.tracer.color) end
            b.Width0 = s.tracer.thick
            b.Width1 = s.tracer.thick * 0.5
